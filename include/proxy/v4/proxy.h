...
template <facade F> class ___PRO4_ENFORCE_EBO proxy;
...
struct copy_dispatch {
  ...
  ___PRO4_STATIC_CALL(void, T&& self, proxy<F>& rhs)
...
class ___PRO4_ENFORCE_EBO allocated_ptr
...
template <class T, class Alloc>
struct ___PRO4_ENFORCE_EBO compact_ptr_storage
...
struct upward_conversion_dispatch : cast_dispatch_base<false, true> {
  ...
  ___PRO4_STATIC_CALL(T&&, T&& self) noexcept { return std::forward<T>(self); }
};
...
struct proxy_cast_accessor_impl {
  ...
  friend T proxy_cast(_Self self) {
    ...
    if constexpr (std::is_lvalue_reference_v<T>) {
      ...
      if (result == nullptr) [[unlikely]] { ___PRO4_THROW(bad_proxy_cast{}); }
      ...
      proxy_invoke<IsDirect, D, O>(
          access_proxy<F>(std::forward<_Self>(self)), ctx);
      if (!result.has_value()) [[unlikely]] { ___PRO4_THROW(bad_proxy_cast{}); }
