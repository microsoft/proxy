...
#define ___PRO_4__STATIC_CALL(__R, ...) static __R operator()(__VA_ARGS__)
#else
#define ___PRO_4__STATIC_CALL(__R, ...) __R operator()(__VA_ARGS__) const
#endif  // __cpp_static_call_operator >= 202207L

#ifdef _MSC_VER
#define ___PRO_4__ENFORCE_EBO __declspec(empty_bases)
#else
#define ___PRO_4__ENFORCE_EBO
#endif  // _MSC_VER

#ifdef NDEBUG
#define ___PRO_4__DEBUG(...)
#else
...
#define ___PRO_4__DIRECT_FUNC_IMPL(...) \
    noexcept(noexcept(__VA_ARGS__)) requires(requires { __VA_ARGS__; }) \
    { return __VA_ARGS__; }

#define ___PRO_4__DEF_MEM_ACCESSOR_TEMPLATE(__MACRO, ...) \
...
#define ___PRO_4__EXPAND_IMPL(__X) __X

#define ___PRO_4__EXPAND_MACRO_IMPL(__MACRO, __1, __2, __3, __NAME, ...) \
    __MACRO##_##__NAME
#define ___PRO_4__EXPAND_MACRO(__MACRO, ...) \
    ___PRO_4__EXPAND_IMPL(___PRO_4__EXPAND_MACRO_IMPL( \
        __MACRO, __VA_ARGS__, 3, 2)(__VA_ARGS__))

////////

#define ___PRO_4__DEF_MEM_ACCESSOR(__Q, __SELF, ...) \
...
#define ___PRO_4__DEF_MEM_DISPATCH_IMPL(__NAME, __FUNC, __FNAME, __TTYPE) \
    struct __NAME { \
      template <__TTYPE __T, class... __Args> \
      ___PRO_4__STATIC_CALL(decltype(auto), __T&& __self, __Args&&... __args) \
          ___PRO_4__DIRECT_FUNC_IMPL(::std::forward<__T>(__self) \
              .__FUNC(::std::forward<__Args>(__args)...)) \
      ___PRO_4__DEF_MEM_ACCESSOR_TEMPLATE(___PRO_4__DEF_MEM_ACCESSOR, __FNAME) \
    }
...
#define PRO_4_DEF_MEM_DISPATCH(__NAME, ...) \
    ___PRO_4__EXPAND_MACRO(___PRO_4__DEF_MEM_DISPATCH, __NAME, __VA_ARGS__)

#define ___PRO_4__DEF_FREE_ACCESSOR(__Q, __NE, __SELF_ARG, __SELF, ...) \
    template <class __F, bool __IsDirect, class __D, class __R, \
        class... __Args> \
    struct accessor<__F, __IsDirect, __D, __R(__Args...) __Q> { \
...
#define ___PRO_4__DEF_FREE_DISPATCH_IMPL(__NAME, __FUNC, __FNAME, __TTYPE) \
    struct __NAME { \
      template <__TTYPE __T, class... __Args> \
      ___PRO_4__STATIC_CALL(decltype(auto), __T&& __self, __Args&&... __args) \
          ___PRO_4__DIRECT_FUNC_IMPL(__FUNC(::std::forward<__T>(__self), \
              ::std::forward<__Args>(__args)...)) \
      ___PRO_4__DEF_FREE_ACCESSOR_TEMPLATE(___PRO_4__DEF_FREE_ACCESSOR, __FNAME) \
    }
...
#define PRO_4_DEF_FREE_DISPATCH(__NAME, ...) \
    ___PRO_4__EXPAND_MACRO(___PRO_4__DEF_FREE_DISPATCH, __NAME, __VA_ARGS__)

#define ___PRO_4__DEF_FREE_AS_MEM_DISPATCH_IMPL(__NAME, __FUNC, __FNAME) \
    struct __NAME { \
...
#define ___PRO_4__DEF_FREE_AS_MEM_DISPATCH_2(__NAME, __FUNC) \
    ___PRO_4__DEF_FREE_AS_MEM_DISPATCH_IMPL(__NAME, __FUNC, __FUNC)
#define ___PRO_4__DEF_FREE_AS_MEM_DISPATCH_3(__NAME, __FUNC, __FNAME) \
    ___PRO_4__DEF_FREE_AS_MEM_DISPATCH_IMPL(__NAME, __FUNC, __FNAME)
#define PRO_4_DEF_FREE_AS_MEM_DISPATCH(__NAME, ...) \
    ___PRO_4__EXPAND_MACRO(___PRO_4__DEF_FREE_AS_MEM_DISPATCH, __NAME, __VA_ARGS__)
