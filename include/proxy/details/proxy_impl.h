...
template <template <class...> class R, class... Args> struct reduction_traits {
  template <class O, class I> using type = typename R<Args..., O, I>::type;
};

template <class Expr> consteval bool is_consteval(Expr) {
...
template <class T> consteval bool is_tuple_like_well_formed() {
...
template <template <class> class O> struct facade_aware_overload_t {
...
template <class T> consteval bool has_copyability(constraint_level level) {
  switch (level) {
...
template <class T> consteval bool has_relocatability(constraint_level level) {
  switch (level) {
  case constraint_level::none:
    return true;
  case constraint_level::nontrivial:
...
template <class T> consteval bool has_destructibility(constraint_level level) {
  switch (level) {
  case constraint_level::none:
    return true;
  case constraint_level::nontrivial:
...
template <class F> struct proxy_helper {
...
template <class F, class P> class proxy_resetting_guard {
public:
...
template <class D, class R, class... Args> R invoke_dispatch(Args &&...args) {
...
template <bool IsDirect, class P> decltype(auto) get_operand(P &&ptr) {
...
template <class O> struct overload_substitution_traits : inapplicable_traits {
...
template <class F, bool IsDirect, class D, class O> struct invocation_meta {
...
template <class... Ms> struct composite_meta_impl : Ms... {
...
template <class T> consteval bool is_is_direct_well_formed() {
  if constexpr (requires {
...
struct conv_traits_impl<C, F, Os...> : applicable_traits {
  ...
  template <class P> static consteval bool diagnose_proxiable() {
    bool verdict = true;
...
template <bool IsDirect, class R> struct refl_meta {
  template <class P>
    requires(IsDirect)
...
template <class R> struct refl_traits {
  using meta = refl_meta<R::is_direct, typename R::reflector_type>;

  template <class P, class F> static consteval bool diagnose_proxiable() {
...
template <class T> struct accessor_traits_impl : std::type_identity<void> {};
template <class T>
...
template <class F> consteval bool is_facade_constraints_well_formed() {
  if constexpr (requires {
                  {
                    F::constraints
...
struct facade_conv_traits_impl<F, Cs...> : applicable_traits {
  ...
  template <class P> static consteval bool diagnose_proxiable_conv() {
    bool verdict = true;
...
template <class F, class... Rs> struct facade_refl_traits_impl {
  ...
  template <class P> static consteval bool diagnose_proxiable_refl() {
    bool verdict = true;
...
struct facade_traits<F>
  ...
  template <class P> static consteval void diagnose_proxiable() {
    bool verdict = true;
...
template <class M> struct meta_ptr_indirect_impl {
...
template <class M, class DM> struct meta_ptr_direct_impl : private M {
  using M::M;
...
template <class T> class inplace_ptr {
public:
  template <class... Args>
...
template <facade F, class A> proxy<F> &access_proxy(A &a) noexcept {
  return details::access_impl<F, A, details::qualifier_type::lv>(a);
}
template <facade F, class A> const proxy<F> &access_proxy(const A &a) noexcept {
...
private:
  template <class P, class... Args> constexpr P &initialize(Args &&...args) {
  ...
  ___PRO_DEBUG(static inline void _symbol_guard(proxy &self,
...
template <class LR, class CLR, class RR, class CRR> struct observer_ptr {
public:
...
template <class Alloc, class T> void deallocate(const Alloc &alloc, T *ptr) {
  auto al =
      typename std::allocator_traits<Alloc>::template rebind_alloc<T>(alloc);
  std::destroy_at(ptr);
  al.deallocate(ptr, 1);
}
template <class Alloc> struct alloc_aware {
public:
...
template <class T> class indirect_ptr {
public:
...
template <class T, class Alloc>
struct ___PRO_ENFORCE_EBO compact_ptr_storage : alloc_aware<Alloc>,
                                                inplace_ptr<T> {
  template <class... Args>
...
template <class F> struct proxy_arg_traits<proxy<F>> : applicable_traits {};
template <class F>
...
template <bool Expl, bool Nullable> struct cast_dispatch_base {
...
struct upward_conversion_dispatch : cast_dispatch_base<false, true> {
  template <non_proxy_arg T> ___PRO_STATIC_CALL(T &&, T &&self) noexcept {
    return std::forward<T>(self);
  }
};

template <class T> struct explicit_conversion_adapter {
...
template <bool IsDirect, class D, class... Os> struct conv_impl {
...
template <bool IsDirect, class R> struct refl_impl {
  static constexpr bool is_direct = IsDirect;
  using reflector_type = R;
  template <class F> using accessor = instantiated_accessor_t<R, F, IsDirect>;
};
template <class Cs, class Rs, proxiable_ptr_constraints C> struct facade_impl {
...
template <bool IsDirect, class D> struct merge_conv_traits {
...
template <class F, class... Cs> struct observer_facade_conv_impl {
...
template <class F, class... Cs> struct weak_facade_impl {
  using convention_types = recursive_reduction_t<
...
struct basic_facade_builder {
  ...
  template <class R> using add_reflection = add_indirect_reflection<R>;
...
template <class F> using view_conversion_overload = proxy_view<F>() noexcept;
...
struct proxy_cast_accessor_impl {
  ...
  template <class T> friend T proxy_cast(_Self self) {
...
template <std::size_t N> struct sign {
...
template <std::size_t N> sign(const char (&str)[N]) -> sign<N>;

struct wildcard {
  wildcard() = delete;

  template <class T> [[noreturn]] operator T() const {
...
template <class FB> using rtti = indirect_rtti<FB>;
#endif // __cpp_rtti >= 199711L
...
template <details::sign Sign, bool Rhs = false> struct operator_dispatch;
...
#define ___PRO_LHS_BINARY_OP_DISPATCH_BODY_IMPL(...)                           \
  ...
  ___PRO_DIRECT_FUNC_IMPL(std::forward<T>(self)                                \
...
#define ___PRO_RHS_OP_DISPATCH_IMPL(...)                                       \
  template <> struct operator_dispatch<#__VA_ARGS__, true> {                   \
    template <details::non_proxy_arg T, class Arg>                             \
    ___PRO_STATIC_CALL(decltype(auto), T &&self, Arg &&arg)                    \
    ___PRO_DIRECT_FUNC_IMPL(std::forward<Arg>(arg)                             \
...
template <> struct operator_dispatch<"()", false> {
...
template <class D> struct weak_dispatch : D {
  using D::operator();
  template <class... Args>
...
struct formatter<pro::proxy_indirect_accessor<F>, CharT> {
  ...
  OutIt format(const pro::proxy_indirect_accessor<F> &ia,
    ...
    return pro::proxy_invoke<false, pro::details::format_dispatch,
                             pro::details::format_overload_t<CharT>>(p, spec_,
                                                                     fc);
  }
